...
if(isset($_POST['reject'])){
    ...
    $job_id=$_GET['job_id'];
	$applied_user_id=$_POST['applied_user_id'];
	$sql="INSERT INTO `rejected_users` (`user_id`) VALUES('$applied_user_id')";
	mysql_query($sql) or die("ddnt wrk");
	$sql="DELETE FROM `requested_for_jobs` WHERE `requested_id`='$applied_user_id' AND `job_id`='$job_id' AND `requested_to`='$profile_id'";
	mysql_query($sql) or die("couldnt  delete");
	header("location:employee.php");
...
echo '<table class="table">';
	    	...
	    	<input type="hidden" name="applied_user_id" value="<?php echo $applied_user_id; ?>"/>
	    	<input type="submit" id="button1" name="select" value="select" />
	    	</form>
	    	<form action="" method="POST">
	    	<input type="hidden" name="applied_user_id" value="<?php echo $applied_user_id; ?>"/>
	    	<input type="submit" id="button2" name="reject" value="reject" />
	    	</form>	
	    	<?php
